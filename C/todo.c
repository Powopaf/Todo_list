...
int add(char* task, char* day, char* month, char* year) {
    ...
    if (file == NULL) { printf("\033[0;31mFailed to open file data (fopen() error)\033[0m\n"); return EXIT_FAILURE; }
    
    size_t length = strlen(task) +strlen(day) + strlen(month) + strlen(year) + 5;
    char* task_to_add = malloc(sizeof(char) * length);
    if (task_to_add == NULL) { printf("\033[0;31mFailed To add task %s (malloc failure)\033[0m\n", task); return EXIT_FAILURE; }

    if (sprintf(task_to_add, "%s %s %s %s\n", task, day, month, year) < 0) { printf("\033[0;31mFail to build string with sprintf()\033[0m\n"); return EXIT_FAILURE; }
    
    if (fprintf(file, task_to_add) < 0) { printf("\033[0;31mFail To write in file (fprintf() error\033[0m\n"); return EXIT_FAILURE; }
...
int help() {
    const char* help_add = "To add a new task the argument must be :\n\033[0;35madd\033[0m \033[0;34mtask day month year\033[0m";
    return EXIT_SUCCESS;
}
